...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        unpack_hiddens,seq_len=pad_packed_sequence(enc_hiddens)
        enc_hiddens=unpack_hiddens.permute(1,0,2)
        
        temp_decode_h=last_hidden.permute(1,0,2)
        temp_decode_h=temp_decode_h.contiguous().view(temp_decode_h.shape[0],-1)
        init_decoder_hidden=self.h_projection(temp_decode_h)
        
        temp_decode_c=last_cell.permute(1,0,2)
        temp_decode_c=temp_decode_c.contiguous().view(temp_decode_c.shape[0],-1)
        init_decoder_cell=self.c_projection(temp_decode_c)
