...
def pad_sents_char(sents, char_pad_token):
    ...
    for i in range(0,len(sents)):
        ...
        for j in range(0,len(sents[i])):
            sents_padded[-1].append([])
            for k in range(0,min(max_word_length,len(sents[i][j]))):
                sents_padded[-1][-1].append(sents[i][j][k])
            for k in range(len(sents[i][j]),max_word_length):
                sents_padded[-1][-1].append(char_pad_token)
        for _ in range(0,max_length-len(sents[i])):
            sents_padded[-1].append([char_pad_token for _ in range(0,max_word_length ) ])
...
def pad_sents(sents, pad_token):
    ...
    for i in range(0,len(sents)):
        sents_padded.append([])
        for j in range(0,len(sents[i])):
            sents_padded[-1].append(sents[i][j])
        for _ in range(0,max_length-len(sents[i])):
