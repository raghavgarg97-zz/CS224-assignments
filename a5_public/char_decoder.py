...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        #print("Enter with hidden_size "+str(hidden_size)+" and target_vocab "+str(len(target_vocab.char2id)))
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        x_processed,dec_hidden=self.forward(x_input,dec_hidden)
        return loss(x_processed.permute(1,2,0),x_target.permute(1,0))
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        output_words=[]
        decoded_words=[]
        for _ in range(0,initialStates[0].shape[1]):
            decoded_words.append('')
        start_idx=self.target_vocab.start_of_word
        end_idx=self.target_vocab.end_of_word
        dec_hidden=initialStates
        
        lst=[start_idx for _ in range(0,dec_hidden[0].shape[1])]
        x_in=torch.unsqueeze(torch.tensor(lst,dtype=torch.long,device=device),dim=0)        

        for _ in range(0,max_length):
            scores,dec_hidden=self.forward(x_in,dec_hidden)
            max_i=torch.argmax(scores,dim=-1)
            output_words.append(max_i)
            x_in=max_i
                
        for i in range(0,len(decoded_words)):
             for ex in output_words:
                if ex[0][i].item()!=end_idx:
                    decoded_words[i]+=self.target_vocab.id2char[ex[0][i].item()]
                else:
                    break
